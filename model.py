    ...
    def run_optim(self, batch_A_imgs, batch_B_imgs, counter, start_time, batch_idx, is_paired):
        ...
        if ("wgan" in self.GAN_type and batch_idx % self.n_critic == 0) or "wgan" not in self.GAN_type:
            ...
            if batch_idx % self.log_freq == 0:
                ...
                if is_paired:
                    loss_info = (Ad, Ag, Bd, Bg)
                    print("time: %4.4f, Ad: %.2f, Ag: %.2f, Bd: %.2f, Bg: %.2f" % (elapsed_time, *loss_info))
                else: 
                    loss_info = (Ad, Ag, Bd, Bg, ABloss, BAloss, cycle_A_loss, cycle_B_loss)
                    print("time: %4.4f, Ad: %.2f, Ag: %.2f, Bd: %.2f, Bg: %.2f,  BAloss: %.5f, ABloss: %.5f cycle_A_loss %.5f cycle_B_loss %.5f" % (elapsed_time, *loss_info))
